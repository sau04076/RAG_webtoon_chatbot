...
def hugging_llm(prompt, model="mistralai/Mistral-7B-Instruct-v0.1"):
    ...
    generate_text = pipeline(task="text-generation", model=model, tokenizer=tokenizer,
                        ...
                        repetition_penalty=1.1, model_kwargs={"device_map": "auto", "max_length":1200, "temperature":0.01})

    generate_text(prompt, max_length=30, num_return_sequences=1)
...
def chat_gpt(prompt, model="gpt-3.5-turbo", max_tokens=4096, max_context_tokens=4096, safety_margin=5):
    ...
    chat_completion = client.chat.completions.create(
        messages=[{"role": "system", 
                   "content": '''Your name is Wakebot, and you are a friendly, kind and helpful friend.
                   Wakebot means 'Webtoon recommendation with knowledge engine chatbot'. It is a service that can be a wakeboard for surfing in the vast ocean of information called webtoon.
...
#%%
def main():
    logo_img = load_image('wakebot.png')
    manual_img = load_image('wakebot_설명서.png')
    info_img = load_image('소개글.png')
    ...
    if submit_button:
        ...
        반말 써줘!
        일상적인 언어로 자연스럽게 표현해주고 쉬운 말로 풀어서 써줘.
        '''

        answer = hugging_llm(prompt)
